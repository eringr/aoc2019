...
fn cycle(mem: &mut [i32; MEM_LEN], pc: usize) -> RunMode {
    let inst = mem[pc];
    ...
    let get_param = |param: i32, encoding: i32| -> i32 {
        return if encoding == 0 {mem[param as usize]} else {param};
    };
    let get_params = || {
        (get_param(mem[pc+1], mode_p1), get_param(mem[pc+2], mode_p2))
    };

    match opcode {
        x if x == Opcodes::Add as i32 => {
            let (p1, p2) = get_params();
            mem[mem[pc+3] as usize] = p1 + p2;
            return RunMode::Running(pc+4);
        },
        x if x == Opcodes::Mul as i32 => {
            let (p1, p2) = get_params();
            mem[mem[pc+3] as usize] = p1 * p2;
            return RunMode::Running(pc+4);
        },
        x if x == Opcodes::In as i32 => {
            println!("Input: 5");
            mem[mem[pc+1] as usize] = 5;
            return RunMode::Running(pc+2);
        },
        x if x == Opcodes::Out as i32 => {
            let p1 = get_param(mem[pc+1], mode_p1);
            println!("Output: {}", p1);
            return RunMode::Running(pc+2);
        },
        x if x == Opcodes::Bne as i32 => {
            let (p1, p2) = get_params();
            return if p1 != 0i32 {RunMode::Running(p2 as usize)} else
                {RunMode::Running(pc+3)};
        },
        x if x == Opcodes::Beq as i32 => {
            let (p1, p2) = get_params();
            return if p1 == 0i32 {RunMode::Running(p2 as usize)} else
                {RunMode::Running(pc+3)};
        },
        x if x == Opcodes::Slt as i32 => {
            let (p1, p2) = get_params();
            mem[mem[pc+3] as usize] = if p1 < p2 {1} else {0};
            return RunMode::Running(pc+4);
        },
        x if x == Opcodes::Seq as i32 => {
            let (p1, p2) = get_params();
            mem[mem[pc+3] as usize] = if p1 == p2 {1} else {0};
            return RunMode::Running(pc+4);
        }
        _ => return RunMode::Halted,
    }
}

fn main() {
    let mut mem = STARTING_MEM.clone();
    let mut pc = RunMode::Running(0);
    while let RunMode::Running(a) = pc {
        pc = cycle(&mut mem, a);
    }
    println!("{}", mem[0]);
}
